...
int32_t main(){
	...
	for(int i=0;i<5*n;++i){
		for(int j=0;j<5*n;++j){
			if(i<n and j<n)a[i][j]=a[i][j];
			else if(i<n and j>=n)a[i][j]=a[i][j-n]+1;
			else a[i][j]=a[i-n][j]+1;
			a[i][j]=a[i][j]%10;
			if(a[i][j]==0)a[i][j]++;
		}
	}
	...
	while(!pq.empty()){
		...
		for(int i=0;i<4;++i){
			if(x+ax[i]>=5*n or y+ay[i]>=5*n or x+ax[i]<0 or y+ay[i]<0)continue;
